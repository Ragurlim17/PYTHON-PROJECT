...
def alert(n):
    if n == 1:
        label_text.config(text="첫번째 고양이를 선택하셨습니다.", font=("맑은 고딕", 20))
        cat_char.config(image=tk_img1)
    elif n == 2:
        label_text.config(text="두번째 고양이를 선택하셨습니다.", font=("맑은 고딕", 20))
        cat_char.config(image=tk_img2)
    elif n == 3:
        label_text.config(text="세번째 고양이를 선택하셨습니다.", font=("맑은 고딕", 20))
...
def gain_experience(page, exp):
    ...
    if current_exp >= max_exp:
        ...
        current_exp -= max_exp
        max_exp += 100
        label_leveltext.config(text=f"레벨업!\n현재 레벨:{current_level}", font=("맑은 고딕", 15))
        label_leveltext.after(1500, label_leveltext.destroy)
    else:
        label_leveltext.config(text=f"{exp} 경험치를 얻었습니다!\n현재 레벨:{current_level}, 경험치:{current_exp}",
                               font=("맑은 고딕", 15))
        label_leveltext.after(1500, label_leveltext.destroy)
    label_level.config(text=f'현재 레벨:{current_level}, 현재 경험치:{current_exp}', font=("맑은 고딕", 15))
